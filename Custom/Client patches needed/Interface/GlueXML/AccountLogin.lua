...
-- buttons use the in-game red panel-button art instead of tinting.
function AccountLogin_TintButtonsRed()
	for i = 1, #DC_RED_BUTTONS do
		local button = _G[DC_RED_BUTTONS[i]];
		if ( button ) then
			...
			local textures = { button:GetNormalTexture(), button:GetPushedTexture(), button:GetHighlightTexture(), button:GetDisabledTexture() };
			for j = 1, #textures do
			...
			-- The glue templates offset ButtonText by (-3, 3) to match the glue art's
			-- baked drop shadow; the panel-button art has none, so recenter the label.
...
function AccountLogin_OnLoad(self)
	...
	if (IsStreamingTrial()) then
		...
		AccountLogin:SetModel("Interface\\Glues\\Models\\UI_MainMenu_Legion\\UI_MainMenu_Legion.m2");
	end

	AccountLogin_TintButtonsRed();
end
