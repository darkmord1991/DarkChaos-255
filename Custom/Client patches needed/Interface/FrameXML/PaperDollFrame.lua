...
function PlayerTitleFrame_UpdateTitles()
	...
	for i = 1, GetNumTitles() do
		if ( IsTitleKnown(i) ~= 0 ) then
			-- Some custom titles can be known by the player but have no name in
			-- the client's CharTitles.dbc (data drift between server grants and
			-- the deployed DBC). GetTitleName() then returns nil and strtrim()
			-- below would error, so skip any title without a usable name.
			local titleName = GetTitleName(i);
			if ( titleName and strtrim(titleName) ~= "" ) then
				titleCount = titleCount + 1;
				playerTitles[titleCount] = playerTitles[titleCount] or { };
				playerTitles[titleCount].name = strtrim(titleName);
