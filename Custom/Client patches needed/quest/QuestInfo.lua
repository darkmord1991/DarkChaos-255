...
local function QuestInfo_GetRewardItemLink(itemType, itemIndex, fromQuestLog)
	if ( fromQuestLog and type(GetQuestLogItemLink) == "function" ) then
		local ok, link = pcall(GetQuestLogItemLink, itemType, itemIndex);
	...
	if ( type(GetQuestItemLink) == "function" ) then
		local ok, link = pcall(GetQuestItemLink, itemType, itemIndex);
		if ( ok and type(link) == "string" and link ~= "" ) then
			return link;
		end
	end
...
local function QuestInfo_ResolveRewardItemTexture(button, itemType, itemIndex, texture, fromQuestLog)
	if ( texture and texture ~= "" ) then
		return texture;
	end

	local link = QuestInfo_GetRewardItemLink(itemType, itemIndex, fromQuestLog);
	...
	if ( button and button.GetName ) then
		...
		if ( buttonName ) then
			...
			if ( iconTexture and iconTexture.GetTexture ) then
				local existing = iconTexture:GetTexture();
				if ( existing ) then
					return existing;
				end
			end
...
local function QuestInfo_SetRewardItemIcon(button, texture)
	if ( not button ) then
		return;
	end

	local resolvedTexture = texture or "Interface\\Icons\\INV_Misc_QuestionMark";
	local iconTargets = {};
	local seen = {};
	local hasDedicatedRegion = false;

	local function AddIconTarget(region, dedicated)
		if ( not region ) then
			return;
		end
		if ( seen[region] ) then
		...
		seen[region] = true;
		table.insert(iconTargets, region);
		if ( dedicated ) then
			hasDedicatedRegion = true;
		end
	end

	AddIconTarget(button.Icon, true);
	AddIconTarget(button.icon, true);
	if ( button.GetName ) then
		local buttonName = button:GetName();
		if ( buttonName ) then
			AddIconTarget(_G[buttonName.."IconTexture"], true);
			AddIconTarget(_G[buttonName.."Icon"], true);
		end
	end
	if ( button.GetNormalTexture ) then
		AddIconTarget(button:GetNormalTexture(), false);
	end
	if ( button.GetPushedTexture ) then
		AddIconTarget(button:GetPushedTexture(), false);
	end

	if ( #iconTargets == 0 and button.SetNormalTexture ) then
		pcall(button.SetNormalTexture, button, resolvedTexture);
		if ( button.GetNormalTexture ) then
			AddIconTarget(button:GetNormalTexture(), false);
		end
	end

	for i = 1, #iconTargets do
		local icon = iconTargets[i];
	...
	if ( button.GetNormalTexture ) then
		...
		if ( normalTexture and normalTexture.SetAlpha ) then
			normalTexture:SetAlpha(hasDedicatedRegion and 0 or 1);
		end
		if ( normalTexture ) then
			if ( hasDedicatedRegion and normalTexture.Hide ) then
				normalTexture:Hide();
			elseif ( normalTexture.Show ) then
				normalTexture:Show();
			end
		end
	end
	...
	if ( button.GetPushedTexture ) then
		...
		if ( pushedTexture and pushedTexture.SetAlpha ) then
			pushedTexture:SetAlpha(hasDedicatedRegion and 0 or 1);
		end
		if ( pushedTexture ) then
			if ( hasDedicatedRegion and pushedTexture.Hide ) then
				pushedTexture:Hide();
			elseif ( pushedTexture.Show ) then
				pushedTexture:Show();
			end
		end
	end
...
function QuestInfo_ShowRewards()
	...
	if ( numQuestChoices > 0 ) then
		...
		for i = 1, numQuestChoices, 1 do	
			...
			texture = QuestInfo_ResolveRewardItemTexture(questItem, questItem.type, i, texture, QuestInfoFrame.questLog);
			questItem:SetID(i)
			questItem:Show();
			-- For the tooltip
			...
			QuestInfo_SetRewardItemIcon(questItem, texture);
			if ( isUsable ) then
    ...
    local reward = money > 0 or honor > 0 or arenaPoints > 0 or talents > 0 or xp > 0 or playerTitle;
    if ( numQuestRewards > 0 or reward ) then
