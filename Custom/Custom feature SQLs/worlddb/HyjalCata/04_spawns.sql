...
DELETE FROM acore_world.gameobject WHERE map=750 AND id>=@OFF;
...
SELECT c.`id`+@OFF, 750, 4923, 4923, c.`spawnMask`, c.`phaseMask`, c.`equipment_id`, c.`position_x`, c.`position_y`, c.`position_z`, c.`orientation`, c.`spawntimesecs`, c.`wander_distance`, c.`currentwaypoint`, COALESCE(c.`curhealth`,1), COALESCE(c.`curmana`,0), c.`MovementType`, COALESCE(c.`npcflag`,0), COALESCE(c.`unit_flags`,0)
FROM cata_world.creature c WHERE (c.map=1 AND c.zoneId=616);
...
SELECT g.`id`+@OFF, 750, 4923, 4923, g.`spawnMask`, g.`phaseMask`, g.`position_x`, g.`position_y`, g.`position_z`, g.`orientation`, g.`rotation0`, g.`rotation1`, g.`rotation2`, g.`rotation3`, g.`spawntimesecs`, g.`animprogress`, g.`state`
FROM cata_world.gameobject g WHERE (g.map=1 AND g.zoneId=616);
