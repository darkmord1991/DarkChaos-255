-- DarkChaos: Item Upgrade Synthesis schema fix
-- Fixes startup abort: ER_BAD_FIELD_ERROR (1054) Unknown column 'recipe_id'
-- and aligns synthesis inputs with current C++ expectations.

-- NOTE: This is placed in pending_db_world so it is picked up by the standard db update/import pipeline.
...
CREATE TABLE `dc_item_upgrade_synthesis_inputs` (
...
COMMENT='Required input items per synthesis recipe';

-- Optional sample recipes (safe defaults; can be removed/edited as needed)
