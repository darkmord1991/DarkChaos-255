-- Worgoblin: starting action bars (mod-worgoblin rows for their classes,
-- ARAC-clone rows for the classes they omit: goblin pal/druid, worgen pal/shaman)
DELETE FROM `playercreateinfo_action` WHERE `race` IN (9, 12);
INSERT INTO `playercreateinfo_action` (`race`, `class`, `button`, `action`) VALUES
(9, 1, 72, 6603),
(9, 1, 73, 78),
(9, 1, 81, 69070),
(9, 1, 82, 69041),
(9, 1, 84, 6603),
(9, 1, 96, 6603),
(9, 3, 0, 6603),
(9, 3, 1, 2973),
(9, 3, 2, 75),
(9, 3, 9, 69070),
(9, 3, 10, 69041),
(9, 4, 0, 6603),
(9, 4, 1, 1752),
(9, 4, 2, 2098),
(9, 4, 3, 2764),
(9, 4, 9, 69070),
(9, 4, 10, 69041),
(9, 5, 0, 585),
(9, 5, 1, 2050),
(9, 5, 9, 69070),
(9, 5, 10, 69041),
(9, 6, 0, 6603),
(9, 6, 1, 49576),
(9, 6, 2, 45477),
(9, 6, 3, 45462),
(9, 6, 4, 45902),
(9, 6, 5, 47541),
(9, 6, 9, 69070),
(9, 6, 11, 69046),
(9, 6, 10, 69041),
(9, 7, 0, 6603),
(9, 7, 1, 403),
(9, 7, 2, 331),
(9, 7, 9, 69070),
(9, 7, 10, 69041),
(9, 8, 0, 133),
(9, 8, 1, 168),
(9, 8, 9, 69070),
(9, 8, 10, 69041),
(9, 9, 0, 686),
(9, 9, 1, 687),
(9, 9, 9, 69070),
(9, 9, 10, 69041),
(12, 1, 72, 6603),
(12, 1, 73, 78),
(12, 1, 84, 6603),
(12, 1, 96, 6603),
-- (12, 1, 120, 68992), Darkflight Warrior
-- (12, 1, 120, 68996), Two Forms Warrior
-- (12, 1, 120, 87840), Running Wild Warrior
(12, 3, 0, 6603),
(12, 3, 1, 2973),
(12, 3, 2, 75),
-- (12, 3, 120, 68992), Darkflight Hunter
-- (12, 3, 120, 68996), Two Forms Hunter
-- (12, 3, 120, 87840), Running Wild Hunter
(12, 4, 0, 6603),
(12, 4, 1, 1752),
(12, 4, 2, 2098),
(12, 4, 3, 2764),
-- (12, 4, 120, 68992), Darkflight Rogue
-- (12, 4, 120, 68996), Two Forms Rogue
-- (12, 4, 120, 87840), Running Wild Rogue
(12, 5, 0, 585),
(12, 5, 1, 2050),
-- (12, 5, 120, 68992), Darkflight Priest
-- (12, 5, 120, 68996), Two Forms Priest
-- (12, 5, 120, 87840), Running Wild Priest
(12, 6, 0, 6603),
(12, 6, 1, 49576),
(12, 6, 2, 45477),
(12, 6, 3, 45462),
(12, 6, 4, 45902),
(12, 6, 5, 47541),
(12, 6, 9, 68992),
-- (12, 6, 120, 68996), Two Forms Death Knight
-- (12, 6, 120, 87840), Running Wild Death Knight
(12, 8, 0, 133),
(12, 8, 1, 168),
-- (12, 8, 120, 68992), Darkflight Mage
-- (12, 8, 120, 68996), Two Forms Mage
-- (12, 8, 120, 87840), Running Wild Mage
(12, 9, 0, 686),
(12, 9, 1, 687),
-- (12, 9, 120, 68992), Darkflight Warlock
-- (12, 9, 120, 68996), Two Forms Warlock
-- (12, 9, 120, 87840), Running Wild Warlock
(12, 11, 0, 5176),
(12, 11, 1, 5185),
(12, 11, 72, 6603),
(12, 11, 96, 6603);

INSERT INTO `playercreateinfo_action` (`race`, `class`, `button`, `action`)
SELECT 9, `class`, `button`, `action` FROM `playercreateinfo_action` WHERE `race` = 2 AND `class` IN (2, 11);
INSERT INTO `playercreateinfo_action` (`race`, `class`, `button`, `action`)
SELECT 12, `class`, `button`, `action` FROM `playercreateinfo_action` WHERE `race` = 1 AND `class` IN (2, 7);
