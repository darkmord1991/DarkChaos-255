-- =====================================================================================
-- Emerald Sanctum (map 824) -- DC difficulty profile, access gates and registries
--
...
-- map-id range (33-560 vanilla, 574-650 WotLK) and 824 falls outside both, so leaving them
...
DELETE FROM `dc_dungeon_entrances` WHERE `dungeon_map` = 824;
DELETE FROM `dc_dungeon_setup` WHERE `map_id` = 824;
DELETE FROM `dc_dungeon_mythic_profile` WHERE `map_id` = 824;
...
INSERT INTO `dc_dungeon_mythic_profile`
    ...
    (824, 'Emerald Sanctum', 1, 1, 2.00, 1.60, 0, 0, 0, 0, 0, 0, 20, 5, 450, 300311);
...
INSERT INTO `dc_dungeon_setup`
    ...
    (824, 'Emerald Sanctum', 0, 1, 1, 1, 2, 1, 0, 1,
     '20-man raid with 2 encounters, the second a weekly-rotating Wakener');
...
DELETE FROM `dc_mplus_dungeons` WHERE `dungeon_id` = 824;
...
INSERT INTO `dungeon_access_template` (`id`, `map_id`, `difficulty`, `min_level`, `max_level`, `min_avg_item_level`, `comment`) VALUES
    (151, 824, 0, 128, 0, 0, 'Emerald Sanctum - Normal'),
    (156, 824, 1, 130, 0, 0, 'Emerald Sanctum - Heroic'),
    (157, 824, 2, 130, 0, 0, 'Emerald Sanctum - Mythic');
...
INSERT INTO `dc_dungeon_npc_mapping`
    ...
    (824, 700100, 'Emerald Sanctum', 0, 128, 130, 1, 16466);

-- -------------------------------------------------------------------------------------
-- Report
-- -------------------------------------------------------------------------------------
SELECT 'mythic_profile (want 1)' AS `check`, CAST(COUNT(*) AS CHAR) AS result FROM `dc_dungeon_mythic_profile` WHERE `map_id` = 824
UNION ALL SELECT 'dungeon_setup (want 1)', CAST(COUNT(*) AS CHAR) FROM `dc_dungeon_setup` WHERE `map_id` = 824
UNION ALL SELECT 'dungeon_entrances (want 1)', CAST(COUNT(*) AS CHAR) FROM `dc_dungeon_entrances` WHERE `dungeon_map` = 824
UNION ALL SELECT 'mplus_dungeons (want 0)', CAST(COUNT(*) AS CHAR) FROM `dc_mplus_dungeons` WHERE `dungeon_id` = 824
UNION ALL SELECT 'access rows (want 3)', CAST(COUNT(*) AS CHAR) FROM `dungeon_access_template` WHERE `map_id` = 824
UNION ALL SELECT 'npc_mapping (want 1)', CAST(COUNT(*) AS CHAR) FROM `dc_dungeon_npc_mapping` WHERE `map_id` = 824
UNION ALL SELECT 'access id used by another map (want 0)', CAST(COUNT(*) AS CHAR)
    FROM `dungeon_access_template` WHERE `id` IN (151, 156, 157) AND `map_id` <> 824;
