...
JOIN (SELECT DISTINCT ct.gossip_menu_id AS mid
  ...
  WHERE ct.gossip_menu_id > 0 AND (c.map=0 AND c.zoneId IN (139,28)) AND ac.entry IS NULL) M ON M.mid = gm.MenuID;
...
JOIN (SELECT DISTINCT gm.TextID AS tid FROM cata_world.gossip_menu gm JOIN (SELECT DISTINCT ct.gossip_menu_id AS mid
  ...
  JOIN cata_world.creature c ON c.id = ct.entry
  LEFT JOIN acore_world.creature_template ac ON ac.entry = ct.entry AND ac.entry < @OFF
  WHERE ct.gossip_menu_id > 0 AND (c.map=0 AND c.zoneId IN (139,28)) AND ac.entry IS NULL) M ON M.mid = gm.MenuID WHERE gm.TextID>0) T ON T.tid = nt.ID;
...
JOIN (SELECT DISTINCT ct.gossip_menu_id AS mid
  ...
  JOIN cata_world.creature c ON c.id = ct.entry
  LEFT JOIN acore_world.creature_template ac ON ac.entry = ct.entry AND ac.entry < @OFF
  WHERE ct.gossip_menu_id > 0 AND (c.map=0 AND c.zoneId IN (139,28)) AND ac.entry IS NULL) M ON M.mid = gmo.MenuID;
