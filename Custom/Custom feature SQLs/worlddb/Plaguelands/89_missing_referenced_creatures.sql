...
--
-- 53 creature templates the band references but that were NEVER IMPORTED IN ANY FORM
...
-- 41 of these 53 have ZERO spawns in Cata.
--
...
--   * all 53 ids were verified free in our creature_template
...
--      None of these 53 have a DC script.
...
DELETE FROM `creature_template` WHERE `entry` IN (
    38887,38895,38923,38980,38981,39002,39038,39098,42597,43007,43236,43541,44175,
    44369,44432,44433,44474,44476,44622,44623,44624,44625,44629,44882,44915,44942,
    45270,45430,45495,45756,47442,47443,47444,47697,47752,48290,48684,48752,49189,
    49230,49231,49337,49542,49553,49561,49626,49627,51833,52072,52553,53592,53737,
    53827);
...
--
--    Of the 56 displays these creatures use, 5 are still absent from our DBC:
...
--    these 53 use resolves through all three layers and NO new creature_model_info
--    rows are needed.
--
--    DisplayScale/Probability are 1/1 -- all 53 have scale 1 in Cata and all 1,466
...
-- ===========================================================================
-- 3. creature_template_addon -- 53 rows
--
...
--    Across these 53 the only non-zero packing field is SheathState (0 or 1) and
...
--    ours, and none of these 53 patrol.
-- ===========================================================================
...
-- ===========================================================================
-- 4. creature_equip_template -- 11 rows. Schemas identical (see 87_).
-- ===========================================================================
...
-- ===========================================================================
-- 5. creature_text -- 27 rows. Cata has 14 columns to our 13: it adds `SoundType`
...
-- ===========================================================================
-- 6. smart_scripts -- 30 rows. Ours is 31 columns to Cata's 29: it adds
...
-- ===========================================================================
-- 7. Fix the creature references those scripts carry at the WRONG id
--
--    Caught from the live boot log after 86_/87_/88_ went in:
--        "SmartAIMgr: Event SMART_EVENT_DISTANCE_CREATURE using invalid creature
--         entry 39097, skipped."      (and 38925)
--
--    **SMART_EVENT_DISTANCE_CREATURE is event_type 75 and holds the creature entry
--    in `event_param2`** (`distance{guid, entry, dist, repeat}`) -- a field the
--    earlier reference sweep did not cover, because it only looked at action 12/33
--    param1 and target 9/10/11/19 param1. Worth remembering: creature ids hide in
--    EVENT params too, not just action and target params.
--
--    Neither is a missing template. Both creatures were imported normally, at the
--    **+4,100,000** id (4138925 Sedrick Calston, 4139097 High Priest Benedictus
--    Voss) -- they have Cata spawns, so the spawn-driven import did see them. It is
--    the REFERENCE that was never remapped: the script rows belong to 38923 and
--    39038, which this file imports at their plain ids, and they point at the plain
--    ids of creatures that live at the remapped ones.
--
--    The UPDATE is written as a general repair rather than two literals: any
--    event-75 reference among the rows just imported that resolves ONLY at
--    +4,100,000 is shifted. Today that is exactly the two above (verified), and the
--    guards mean it can never touch a reference that already resolves.
-- ===========================================================================
UPDATE `smart_scripts` s
SET s.`event_param2` = s.`event_param2` + 4100000
WHERE s.`source_type` = 0
  ...
  AND s.`entryorguid` IN (
    ...
    49230,49231,49337,49542,49553,49561,49626,49627,51833,52072,52553,53592,53737,
    53827)
  AND NOT EXISTS (SELECT 1 FROM `creature_template` a WHERE a.`entry` = s.`event_param2`)
  AND EXISTS     (SELECT 1 FROM `creature_template` b WHERE b.`entry` = s.`event_param2` + 4100000);

-- ===========================================================================
-- THE 9 THAT NEEDED ART -- now included, via 90_
--
-- These are the REAL NPCs a player sees, as opposed to the credit bunnies above, so
-- an invisible substitute would have been a visible bug. They were held back until
-- their displays existed. **`90_creature_displays_batch2.sql` supplies all 12 of
-- those displays and MUST be applied before this file.**
--
--     38980  Spirit of Devlin Agamand   display 31249
--     43007  Shadra                     display 33275
--     44369  Lord Godfrey               display 29675
--     44629  Garrosh Hellscream         display 32907   (art downported + packed)
...
--
-- 11 of the 12 reuse a model the DC client already ships, so they needed DBC rows and
-- a baked texture only. Garrosh was the one exception -- CREATURE\GARROSH\GARROSH.M2
-- was absent -- and was downported with wxl-baker to MD20 v264 and packed into
-- patch-G; 7 of its 11 .skin batches carried Cata's 0x8000 shader ids, which a 3.3.5
-- client silently drops, and were repaired.
-- ===========================================================================

-- ---------------------------------------------------------------------------
-- Verification
-- ---------------------------------------------------------------------------
SELECT 'templates imported (want 53)' AS what, COUNT(*) AS n
FROM `creature_template` WHERE `entry` IN (38887,38895,38923,38980,38981,39002,39038,39098,42597,43007,43236,43541,44175,44369,44432,44433,44474,44476,44622,44623,44624,44625,44629,44882,44915,44942,45270,45430,45495,45756,47442,47443,47444,47697,47752,48290,48684,48752,49189,49230,49231,49337,49542,49553,49561,49626,49627,51833,52072,52553,53592,53737,53827)
UNION ALL SELECT 'model rows (want 74)', COUNT(*)
FROM `creature_template_model` WHERE `CreatureID` IN (38887,38895,38923,38980,38981,39002,39038,39098,42597,43007,43236,43541,44175,44369,44432,44433,44474,44476,44622,44623,44624,44625,44629,44882,44915,44942,45270,45430,45495,45756,47442,47443,47444,47697,47752,48290,48684,48752,49189,49230,49231,49337,49542,49553,49561,49626,49627,51833,52072,52553,53592,53737,53827)
UNION ALL SELECT 'addon rows (want 53)', COUNT(*)
FROM `creature_template_addon` WHERE `entry` IN (38887,38895,38923,38980,38981,39002,39038,39098,42597,43007,43236,43541,44175,44369,44432,44433,44474,44476,44622,44623,44624,44625,44629,44882,44915,44942,45270,45430,45495,45756,47442,47443,47444,47697,47752,48290,48684,48752,49189,49230,49231,49337,49542,49553,49561,49626,49627,51833,52072,52553,53592,53737,53827)
UNION ALL SELECT 'creature_text rows (want 27)', COUNT(*)
FROM `creature_text` WHERE `CreatureID` IN (38887,38895,38923,38980,38981,39002,39038,39098,42597,43007,43236,43541,44175,44369,44432,44433,44474,44476,44622,44623,44624,44625,44629,44882,44915,44942,45270,45430,45495,45756,47442,47443,47444,47697,47752,48290,48684,48752,49189,49230,49231,49337,49542,49553,49561,49626,49627,51833,52072,52553,53592,53737,53827)
UNION ALL SELECT 'smart_scripts rows (want 30)', COUNT(*)
FROM `smart_scripts` WHERE `source_type`=0 AND `entryorguid` IN (38887,38895,38923,38980,38981,39002,39038,39098,42597,43007,43236,43541,44175,44369,44432,44433,44474,44476,44622,44623,44624,44625,44629,44882,44915,44942,45270,45430,45495,45756,47442,47443,47444,47697,47752,48290,48684,48752,49189,49230,49231,49337,49542,49553,49561,49626,49627,51833,52072,52553,53592,53737,53827)
UNION ALL SELECT 'quest objectives now resolvable (was 49 broken)',
  (SELECT COUNT(*) FROM `quest_template` q WHERE q.`RequiredNpcOrGo1` IN (38887,38895,38923,38980,38981,39002,39038,39098,42597,43007,43236,43541,44175,44369,44432,44433,44474,44476,44622,44623,44624,44625,44629,44882,44915,44942,45270,45430,45495,45756,47442,47443,47444,47697,47752,48290,48684,48752,49189,49230,49231,49337,49542,49553,49561,49626,49627,51833,52072,52553,53592,53737,53827));

-- must be empty: an event-75 creature reference that resolves at neither id, which
-- is what logs "SMART_EVENT_DISTANCE_CREATURE using invalid creature entry N"
SELECT 'PROBLEM: event-75 creature ref unresolvable' AS problem, s.`entryorguid`, s.`id`, s.`event_param2`
FROM `smart_scripts` s
WHERE s.`source_type` = 0 AND s.`event_type` = 75 AND s.`event_param2` > 0
  AND NOT EXISTS (SELECT 1 FROM `creature_template` t WHERE t.`entry` = s.`event_param2`);
...
WHERE t.`entry` IN (38887,38895,38923,38980,38981,39002,39038,39098,42597,43007,43236,43541,44175,44369,44432,44433,44474,44476,44622,44623,44624,44625,44629,44882,44915,44942,45270,45430,45495,45756,47442,47443,47444,47697,47752,48290,48684,48752,49189,49230,49231,49337,49542,49553,49561,49626,49627,51833,52072,52553,53592,53737,53827)
...
WHERE m.`CreatureID` IN (38887,38895,38923,38980,38981,39002,39038,39098,42597,43007,43236,43541,44175,44369,44432,44433,44474,44476,44622,44623,44624,44625,44629,44882,44915,44942,45270,45430,45495,45756,47442,47443,47444,47697,47752,48290,48684,48752,49189,49230,49231,49337,49542,49553,49561,49626,49627,51833,52072,52553,53592,53737,53827)
