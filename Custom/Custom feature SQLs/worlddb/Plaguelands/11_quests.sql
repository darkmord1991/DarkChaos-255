...
INSERT IGNORE INTO _dc_q (q) SELECT quest FROM cata_world.creature_queststarter WHERE id IN (SELECT DISTINCT c.id FROM cata_world.creature c WHERE (c.map=0 AND c.zoneId IN (139,28)))
  UNION SELECT quest FROM cata_world.creature_questender WHERE id IN (SELECT DISTINCT c.id FROM cata_world.creature c WHERE (c.map=0 AND c.zoneId IN (139,28)));
...
UPDATE acore_world.quest_template SET
  `RequiredNpcOrGo1` = CASE WHEN `RequiredNpcOrGo1`>0 AND `RequiredNpcOrGo1` IN (SELECT DISTINCT c.id FROM cata_world.creature c WHERE (c.map=0 AND c.zoneId IN (139,28))) THEN `RequiredNpcOrGo1`+@OFF WHEN `RequiredNpcOrGo1`<0 AND -`RequiredNpcOrGo1` IN (SELECT DISTINCT g.id FROM cata_world.gameobject g WHERE (g.map=0 AND g.zoneId IN (139,28))) THEN `RequiredNpcOrGo1`-@OFF ELSE `RequiredNpcOrGo1` END,
  `RequiredNpcOrGo2` = CASE WHEN `RequiredNpcOrGo2`>0 AND `RequiredNpcOrGo2` IN (SELECT DISTINCT c.id FROM cata_world.creature c WHERE (c.map=0 AND c.zoneId IN (139,28))) THEN `RequiredNpcOrGo2`+@OFF WHEN `RequiredNpcOrGo2`<0 AND -`RequiredNpcOrGo2` IN (SELECT DISTINCT g.id FROM cata_world.gameobject g WHERE (g.map=0 AND g.zoneId IN (139,28))) THEN `RequiredNpcOrGo2`-@OFF ELSE `RequiredNpcOrGo2` END,
  `RequiredNpcOrGo3` = CASE WHEN `RequiredNpcOrGo3`>0 AND `RequiredNpcOrGo3` IN (SELECT DISTINCT c.id FROM cata_world.creature c WHERE (c.map=0 AND c.zoneId IN (139,28))) THEN `RequiredNpcOrGo3`+@OFF WHEN `RequiredNpcOrGo3`<0 AND -`RequiredNpcOrGo3` IN (SELECT DISTINCT g.id FROM cata_world.gameobject g WHERE (g.map=0 AND g.zoneId IN (139,28))) THEN `RequiredNpcOrGo3`-@OFF ELSE `RequiredNpcOrGo3` END,
  `RequiredNpcOrGo4` = CASE WHEN `RequiredNpcOrGo4`>0 AND `RequiredNpcOrGo4` IN (SELECT DISTINCT c.id FROM cata_world.creature c WHERE (c.map=0 AND c.zoneId IN (139,28))) THEN `RequiredNpcOrGo4`+@OFF WHEN `RequiredNpcOrGo4`<0 AND -`RequiredNpcOrGo4` IN (SELECT DISTINCT g.id FROM cata_world.gameobject g WHERE (g.map=0 AND g.zoneId IN (139,28))) THEN `RequiredNpcOrGo4`-@OFF ELSE `RequiredNpcOrGo4` END
...
INSERT IGNORE INTO acore_world.creature_queststarter (`id`, `quest`) SELECT `id`+@OFF, `quest` FROM cata_world.creature_queststarter WHERE id IN (SELECT DISTINCT c.id FROM cata_world.creature c WHERE (c.map=0 AND c.zoneId IN (139,28))) AND id NOT IN (SELECT entry FROM acore_world.creature_template WHERE entry < @OFF);
INSERT IGNORE INTO acore_world.creature_queststarter (`id`, `quest`) SELECT `id`+@OFF, `quest` FROM acore_world.creature_queststarter WHERE id IN (SELECT DISTINCT c.id FROM cata_world.creature c WHERE (c.map=0 AND c.zoneId IN (139,28))) AND id IN (SELECT entry FROM acore_world.creature_template WHERE entry < @OFF) AND id < @OFF;
INSERT IGNORE INTO acore_world.creature_questender   (`id`, `quest`) SELECT `id`+@OFF, `quest` FROM cata_world.creature_questender   WHERE id IN (SELECT DISTINCT c.id FROM cata_world.creature c WHERE (c.map=0 AND c.zoneId IN (139,28))) AND id NOT IN (SELECT entry FROM acore_world.creature_template WHERE entry < @OFF);
INSERT IGNORE INTO acore_world.creature_questender   (`id`, `quest`) SELECT `id`+@OFF, `quest` FROM acore_world.creature_questender   WHERE id IN (SELECT DISTINCT c.id FROM cata_world.creature c WHERE (c.map=0 AND c.zoneId IN (139,28))) AND id IN (SELECT entry FROM acore_world.creature_template WHERE entry < @OFF) AND id < @OFF;
