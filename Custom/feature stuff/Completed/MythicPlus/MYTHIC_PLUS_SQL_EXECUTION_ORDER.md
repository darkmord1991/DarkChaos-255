...
- `dc_dungeon_setup.season_lock` (FK constraint)
...
- `dc_dungeon_setup` (Vanilla → WotLK unified toggles)
...
- Vanilla/TBC/WotLK dungeon toggles + Season 1 lineup
...
season_lock → dc_mplus_seasons.season_id (FK)
```
...
a foreign key constraint fails (`acore_world`.`dc_dungeon_setup`,
CONSTRAINT `dc_dungeon_setup_ibfk_2` FOREIGN KEY (`season_lock`)
REFERENCES `dc_mplus_seasons` (`season_id`) ON DELETE SET NULL)
```
...
### Check Season 1 Dungeon Flags
```sql
SELECT 
    ds.map_id,
    dmp.name AS dungeon_name,
    ds.mythic_plus_enabled,
    ds.season_lock
FROM dc_dungeon_setup ds
JOIN dc_dungeon_mythic_profile dmp ON ds.map_id = dmp.map_id
WHERE ds.season_lock = 1 AND ds.mythic_plus_enabled = 1
ORDER BY dmp.name;
```
...
```sql
-- Remove Season 1 (affix schedule removed, dungeon flags reset)
...
**Featured Dungeons** (10 WotLK via `dc_dungeon_setup` Season 1 flags):
1. Utgarde Keep (574)
