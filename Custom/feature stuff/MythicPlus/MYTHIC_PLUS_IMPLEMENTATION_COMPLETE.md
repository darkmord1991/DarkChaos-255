...
{
   QueryResult result = WorldDatabase.Query(
      "SELECT 1 FROM dc_dungeon_setup "
      "WHERE map_id = {} "
      "AND mythic_plus_enabled = 1 "
      "AND (season_lock IS NULL OR season_lock = {})",
      mapId, seasonId);
...
2. **dc_dungeon_setup** (Vanilla/TBC/WotLK unified toggles)
   - Stores unlock + difficulty toggles per dungeon (map_id)
   - Season 1 rows flagged with `mythic_plus_enabled = 1` and `season_lock = 1`
   - Foreign keys: map_id → dc_dungeon_mythic_profile(map_id), season_lock → dc_mplus_seasons(season_id)
...
**Cause**: Dungeon row in `dc_dungeon_setup` is disabled or locked to another season
**Fix**: Either flag the dungeon for the active season or set `MythicPlus.FeaturedOnly = 0`
```sql
...
2. **Cache Featured Dungeons**: Load `dc_dungeon_setup` flags at season start, refresh on season change
3. **Index Database**: Ensure indexes on season_id, map_id, week_number

### Recommended Indexes
```sql
CREATE INDEX idx_dungeon_setup_mplus ON dc_dungeon_setup(mythic_plus_enabled, season_lock, map_id);
