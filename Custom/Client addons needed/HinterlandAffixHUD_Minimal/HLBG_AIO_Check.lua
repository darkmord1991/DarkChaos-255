...
-- Helper function for callback with a specific object
function _G.HLBG_RegisterAIOMethodCallback(object, method, name)
    if type(object) ~= "table" or type(method) ~= "string" or type(object[method]) ~= "function" then
        return false
    end
    ...
    return _G.HLBG_RegisterAIOCallback(callback, name or (tostring(object) .. ":" .. method))
end
