...
local addon = {}
addon._lastMapUpdateId = addon._lastMapUpdateId or nil
addon._lastDetectionKey = addon._lastDetectionKey or nil
addon._gpsData = addon._gpsData or {}
addon._gpsChannel = addon._gpsChannel or "DCMAP_GPS"
addon._aioRegistered = addon._aioRegistered or false
addon._aioWarned = addon._aioWarned or false
-- Known textures packaged with the addon (used for one-time diagnostics)
...
local function ShouldLogMapUpdate(currentId)
    if addon._lastMapUpdateId == currentId then return false end
    addon._lastMapUpdateId = currentId
    return true
end

local function ShouldLogDetection(mapId, label)
    local key = tostring(label or "nil") .. ":" .. tostring(mapId or "nil")
    if addon._lastDetectionKey == key then return false end
    addon._lastDetectionKey = key
    return true
end

local function FormatGPSLine(header, value)
    return string.format("  %s: %s", header, value ~= nil and tostring(value) or "n/a")
end

local function FormatCoordComponent(value)
...
local function HandleGPSPayload(payload)
    local gps = addon._gpsData
    ...
    if type(payload) == "table" then
        ...
        assign("map", payload.map, payload.mapId)
        assign("zone", payload.zone, payload.zoneId)
        assign("area", payload.area, payload.areaId)
        assign("x", payload.x, payload.posX)
        assign("y", payload.y, payload.posY)
        assign("z", payload.z, payload.posZ)
        assign("orientation", payload.o, payload.orientation, payload.facing)
        assign("phase", payload.phase, payload.phaseMask)
        assign("instance", payload.instance, payload.instanceId)
        gps.raw = nil
    else
        gps.raw = (payload ~= nil) and tostring(payload) or "nil"
    end

    local numericFields = { "map", "zone", "area", "x", "y", "z", "orientation", "phase", "instance" }
    for _, field in ipairs(numericFields) do
        local value = gps[field]
...
local function InitializeAIO()
    if addon._aioRegistered then return end

    local aio = _G.AIO
    if not (type(aio) == "table" and type(aio.Register) == "function") then
        if not addon._aioWarned and IsDebugEnabled and IsDebugEnabled() then
            addon._aioWarned = true
            DCMap_Debug("AIO not detected; GPS debug feed unavailable")
        end
        return
    end

    local ok, err = pcall(aio.Register, addon._gpsChannel, function(opcode, payload)
        if opcode == "GPS" or opcode == "gps" then
            HandleGPSPayload(payload)
            local detectionKey = string.format("%s:%s", tostring(addon._gpsData.map), tostring(addon._gpsData.zone))
            if ShouldLogDetection(detectionKey, "gps-update") then
                DCMap_Debug(string.format("GPS update -> map=%s zone=%s area=%s x=%s y=%s z=%s phase=%s",
                    tostring(addon._gpsData.map),
                    tostring(addon._gpsData.zone),
                    tostring(addon._gpsData.area),
                    FormatCoordComponent(addon._gpsData.x),
                    FormatCoordComponent(addon._gpsData.y),
                    FormatCoordComponent(addon._gpsData.z),
                    tostring(addon._gpsData.phase)))
            end
        else
            DCMap_Debug("GPS feed: unknown opcode", tostring(opcode))
        end
    end)

    if ok then
        addon._aioRegistered = true
        DCMap_Debug("AIO GPS channel registered; awaiting server data")
        if type(aio.Handle) == "function" then
            pcall(aio.Handle, addon._gpsChannel, "REQUEST")
        end
    else
        DCMap_Debug("Failed to register AIO GPS channel:", tostring(err))
    end
end
...
local function ShowMapBackgroundIfNeeded(mapId)
    ...
    if IsAzsharaCrater(viewedMapId) then
        ok = true
        detectedMap = "azshara"
        if ShouldLogDetection(viewedMapId, "azshara-id") then
    ...
    if IsHyjal(viewedMapId) then
        ok = true
        detectedMap = "hyjal"
        if ShouldLogDetection(viewedMapId, "hyjal-id") then
    ...
    if not ok and type(GetMapInfo) == "function" then
        ...
        if mname:lower():find("azshara") and not mname:lower():find("hyjal") then
            ok = true
            detectedMap = "azshara"
            if ShouldLogDetection(mname, "azshara-name") then
                DCMap_Debug("Detected Azshara by map name: " .. mname)
            end
        elseif mname:lower():find("hyjal") then
            ok = true
            detectedMap = "hyjal"
            if ShouldLogDetection(mname, "hyjal-name") then
    ...
    if not ok then
        ...
        if forcedType == "azshara" or forcedType == "hyjal" then
            ...
            if ShouldLogDetection(forcedType, "forced-map") then
    ...
    if ok then
        if detectedMap then
            SetForcedMapType(detectedMap)
            addon._activeCustomMap = detectedMap
            addon._activeCustomMapId = viewedMapId
        end
        if ShouldLogDetection(detectedMap, "show-map") then
        ...
        if DCMapExtensionDB and DCMapExtensionDB.useStitchedMap then
            ...
            if st and st._currentMap == detectedMap and st:IsShown() then
                ...
                if st.Reflow then pcall(st.Reflow, st) end
                if ShouldLogDetection(detectedMap, "already-loaded") then
        ...
        addon._activeCustomMap = nil
        addon._activeCustomMapId = nil
        ClearForcedMapType()
        addon.background:Hide()
...
eventFrame:SetScript("OnEvent", function(_, event, arg1, arg2)
    ...
    elseif event == "WORLD_MAP_UPDATE" then
        local currentMapID = GetCurrentMapAreaID() or 0
        if ShouldLogMapUpdate(currentMapID) then
...
SlashCmdList["DCMAP"] = function(msg)
    ...
    elseif args[1] == "gps" then
        InitializeAIO()
        local gps = addon._gpsData or {}
        local hasData = gps.timestamp ~= nil
        if DEFAULT_CHAT_FRAME and DEFAULT_CHAT_FRAME.AddMessage then
            pcall(DEFAULT_CHAT_FRAME.AddMessage, DEFAULT_CHAT_FRAME, "|cFF00FFFFDCMap GPS Diagnostics:|r")
            if hasData then
                pcall(DEFAULT_CHAT_FRAME.AddMessage, DEFAULT_CHAT_FRAME, FormatGPSLine("Timestamp", gps.timestamp))
                pcall(DEFAULT_CHAT_FRAME.AddMessage, DEFAULT_CHAT_FRAME, FormatGPSLine("Map", gps.map))
                pcall(DEFAULT_CHAT_FRAME.AddMessage, DEFAULT_CHAT_FRAME, FormatGPSLine("Zone", gps.zone))
                pcall(DEFAULT_CHAT_FRAME.AddMessage, DEFAULT_CHAT_FRAME, FormatGPSLine("Area", gps.area))
                local coords = string.format("%s, %s, %s", FormatCoordComponent(gps.x), FormatCoordComponent(gps.y), FormatCoordComponent(gps.z))
                pcall(DEFAULT_CHAT_FRAME.AddMessage, DEFAULT_CHAT_FRAME, FormatGPSLine("Coords", coords))
                pcall(DEFAULT_CHAT_FRAME.AddMessage, DEFAULT_CHAT_FRAME, FormatGPSLine("Orientation", FormatCoordComponent(gps.orientation)))
                pcall(DEFAULT_CHAT_FRAME.AddMessage, DEFAULT_CHAT_FRAME, FormatGPSLine("Phase", FormatCoordComponent(gps.phase)))
                pcall(DEFAULT_CHAT_FRAME.AddMessage, DEFAULT_CHAT_FRAME, FormatGPSLine("Instance", FormatCoordComponent(gps.instance)))
                if gps.raw then
                    pcall(DEFAULT_CHAT_FRAME.AddMessage, DEFAULT_CHAT_FRAME, FormatGPSLine("Raw", gps.raw))
                end
            else
                pcall(DEFAULT_CHAT_FRAME.AddMessage, DEFAULT_CHAT_FRAME, "  No GPS payload received yet. Use /dcmap gps request after enabling the server feed.")
            end
        end
        if args[2] == "request" then
            local aio = _G.AIO
            if type(aio) == "table" and type(aio.Handle) == "function" then
                local ok, err = pcall(aio.Handle, addon._gpsChannel, "REQUEST")
                if DEFAULT_CHAT_FRAME and DEFAULT_CHAT_FRAME.AddMessage then
                    if ok then
                        pcall(DEFAULT_CHAT_FRAME.AddMessage, DEFAULT_CHAT_FRAME, "DC-MapExtension: requested GPS refresh from server")
                    else
                        pcall(DEFAULT_CHAT_FRAME.AddMessage, DEFAULT_CHAT_FRAME, string.format("DC-MapExtension: failed to request GPS refresh (%s)", tostring(err)))
                    end
                end
            elseif DEFAULT_CHAT_FRAME and DEFAULT_CHAT_FRAME.AddMessage then
                pcall(DEFAULT_CHAT_FRAME.AddMessage, DEFAULT_CHAT_FRAME, "DC-MapExtension: AIO not ready; cannot request GPS update")
            end
        end
    elseif msg == "mapinfo" or msg == "info" then
        ...
        if DEFAULT_CHAT_FRAME and DEFAULT_CHAT_FRAME.AddMessage then pcall(DEFAULT_CHAT_FRAME.AddMessage, DEFAULT_CHAT_FRAME, "DC-MapExtension: /dcmap debug|show|hide|clear|setmap <id>|mapinfo|gps [request]") end
    end
end
