--[[
    DC-TitleFix - Fixes player title picker error
    ...
    Enhanced v2.0 - Multiple protection layers for robust handling
]]

-- Define ALL default titles for 3.3.5a (from actual Wrath titles database)
-- Extracted from CharTitles.csv with complete title list
...
local DEFAULT_TITLES = {
    ...
    [187] = "Prestige X %s",
    [188] = "Dungeon Delver",
    [189] = "Stratholme Conqueror",
    [190] = "Molten Purifier",
    [191] = "Shadow Slayer",
    [192] = "Titan's Foe",
    [193] = "Crusader Supreme",
    [194] = "Lichborne Champion",
    [195] = "Crimson Protector",
    [196] = "Veteran Dungeon Master",
    [197] = "Master Dungeon Conqueror",
    [198] = "Elite Quest Master",
    [199] = "Legendary Explorer",
    [200] = "Speedrunner",
    [201] = "Speed Demon",
    [202] = "Loot Lord",
    [203] = "Collector's Pride",
    [204] = "Gold Rush",
    [205] = "Rich Adventurer",
    [206] = "Token Hoarder",
    [207] = "Token Master",
    [208] = "Blackrock Depths Conqueror",
    [209] = "Stratholme Liberator",
    [210] = "Molten Core Purifier",
    [211] = "Black Temple Destroyer",
    [212] = "Ulduar Titan Slayer",
    [213] = "Trial Champion",
}
...
local function InitializeTitleFix()
    ...
    -- This prevents strtrim() errors in PaperDollFrame
    ...
    if PlayerTitleFrame_UpdateTitles and type(PlayerTitleFrame_UpdateTitles) == "function" then
        ...
        _G.PlayerTitleFrame_UpdateTitles = function()
            ...
            if not success then
                print("|cffff6b6b[DC-TitleFix]|r Error updating titles: " .. tostring(err))
                -- Gracefully degrade: clear the scroll frame
    ...
    if PlayerTitlePickerScrollFrame_Update and type(PlayerTitlePickerScrollFrame_Update) == "function" then
        ...
        _G.PlayerTitlePickerScrollFrame_Update = function()
            ...
            -- Wrap in pcall to catch strtrim errors
            local success, err = pcall(function()
                original_ScrollUpdate()
            end)
            
            if not success then
                print("|cffff6b6b[DC-TitleFix]|r Error updating scroll frame: " .. tostring(err))
                -- Clear the scroll frame if error occurs (graceful degradation)
                if PlayerTitlePickerScrollFrame then
                    FauxScrollFrame_SetOffset(PlayerTitlePickerScrollFrame, 0)
                    for i = 1, 7 do
    ...
    -- LAYER 6: Also patch PaperDollFrame_UpdatePortrait if it tries to use titles
    if PaperDollFrame_UpdatePortrait and type(PaperDollFrame_UpdatePortrait) == "function" then
        ...
        _G.PaperDollFrame_UpdatePortrait = function(frame)
            ...
            local success, err = pcall(function()
                original_UpdatePortrait(frame)
            end)
            
            if not success then
                print("|cffff6b6b[DC-TitleFix]|r Error updating portrait: " .. tostring(err))
            end
        end
    end
end

-- Initialize immediately to catch early frame loading
InitializeTitleFix()

-- Also initialize on addon load
local frame = CreateFrame("Frame")
frame:RegisterEvent("ADDON_LOADED")
frame:RegisterEvent("PLAYER_LOGIN")
frame:RegisterEvent("CHARACTER_SHEET_OPEN")
frame:SetScript("OnEvent", function(self, event, addonName)
    if event == "ADDON_LOADED" then
        -- Re-initialize on every addon load for safety
        InitializeTitleFix()
    elseif event == "PLAYER_LOGIN" then
        -- Re-initialize when player logs in (server sends data at this point)
        InitializeTitleFix()
    elseif event == "CHARACTER_SHEET_OPEN" then
        -- Ensure everything is set up when character sheet opens
        InitializeTitleFix()
    end
end)

-- DEBUG: Print when addon loads
print("|cff00ff00[DC-TitleFix]|r Loaded - Title system protection active")
