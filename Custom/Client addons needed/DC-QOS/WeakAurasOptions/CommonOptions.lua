  ...
  if not withoutheader then
    options[key .. "collapseButton"] = {
      ...
      image = function()
        if notcollapsable then
          return OptionsPrivate.GetTexture("bullet1"), 18, 18
        else
          return isCollapsed() and OptionsPrivate.GetTexture("expand") or OptionsPrivate.GetTexture("collapse"), 18, 18
        end
      end,
    ...
    if hasApplyTemplate then
      options[key .. "applyTemplate"] = {
        ...
        order = order + 0.7,
        width = 0.15,
        image = OptionsPrivate.GetTexture("template"),
        imageWidth = 24,
        imageHeight = 24,
        control = "WeakAurasIcon",
        hidden = hiddenFunc
