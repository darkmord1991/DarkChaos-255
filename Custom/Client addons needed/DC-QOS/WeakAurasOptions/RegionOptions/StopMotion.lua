...
local function GetTexture(name)
  if OptionsPrivate and OptionsPrivate.GetTexture then
    return OptionsPrivate.GetTexture(name)
  end
  return (WeakAuras.TexturePath or "Interface\\AddOns\\DC-QOS\\WeakAuras\\Media\\Textures") .. "\\" .. name
end
...
local function modifyThumbnail(parent, region, data, fullModify, size)
    ...
    if (tdata) then
      ...
      if rows then
        ...
        if rows then
          ...
          if not data.customForegroundFrames then
            local info = texture_data[data.foregroundTexture]
            if info then
              data.customForegroundFrames = info.count
              data.customForegroundRows = info.rows
              data.customForegroundColumns = info.columns
              data.customForegroundFrameWidth = info.frameWidth or 0
              data.customForegroundFrameHeight = info.frameHeight or 0
              data.customForegroundFileWidth = info.fileWidth or 0
              data.customForegroundFileHeight = info.fileHeight or 0
            end
          end

          local customFrames = data.customForegroundFrames or 1
          if customFrames < 1 then customFrames = 1 end
          local lastFrame = customFrames - 1;
          region.startFrame = floor( (data.startPercent or 0) * lastFrame) + 1;
          region.endFrame = floor( (data.endPercent or 1) * lastFrame) + 1;
          region.foreground.rows = data.customForegroundRows or 1;
          region.foreground.columns = data.customForegroundColumns or 1;
    ...
    local texture = data.foregroundTexture or GetTexture("stopmotion");
...
local function createIcon()
    local data = {
        height = 30,
        width = 30,
        foregroundTexture = GetTexture("StopMotion"),
