...
do
  ...
  local textures = Private.texture_types
  if textures then
    for category, entries in pairs(textures) do
      if type(entries) == "table" then
        local remap = {}
        for path, label in pairs(entries) do
          local newPath = (type(path) == "string") and path:gsub("^Interface\\AddOns\\WeakAuras", base) or path
          remap[newPath] = label
        end
        textures[category] = remap
      end
    end
  end
end
