...
function Private.LoadOptions(msg)
  if not(IsAddOnLoaded("WeakAurasOptions")) then
    -- Embedded mode (DC-QOS) loads WeakAurasOptions directly; no standalone addon to load.
    if _G.WeakAurasOptionsLoaded then
      return true;
    end
...
Broker_WeakAuras = LDB:NewDataObject("WeakAuras", {
  type = "launcher",
  text = "WeakAuras",
  icon = (WeakAuras.TexturePath or "Interface\\AddOns\\DC-QOS\\WeakAuras\\Media\\Textures") .. "\\icon.blp",
...
loadedFrame:SetScript("OnEvent", function(self, event, addon)
  if(event == "ADDON_LOADED") then
    ...
    if(addon == ADDON_NAME or addon == "DC-QOS") then
