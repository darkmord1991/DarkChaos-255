...
_G.WeakAurasOptionsLoaded = true
...
OptionsPrivate.MediaPath = (WeakAuras and WeakAuras.MediaPath) or "Interface\\AddOns\\DC-QOS\\WeakAuras\\Media"
OptionsPrivate.TexturePath = OptionsPrivate.MediaPath .. "\\Textures\\"
function OptionsPrivate.GetTexture(file)
  return OptionsPrivate.TexturePath .. file
end

function OptionsPrivate.ResolvePath(path)
  if type(path) ~= "string" then
    return path
  end
  local base = (WeakAuras and WeakAuras.AddonPath) or "Interface\\AddOns\\DC-QOS\\WeakAuras"
  return path:gsub("^Interface\\\\AddOns\\\\WeakAuras", base)
end
...
loadedFrame:SetScript("OnEvent", function(self, event, addon)
  if (event == "ADDON_LOADED") then
    -- Check for both original addon name and DC-QOS (embedded mode)
    if(addon == ADDON_NAME or addon == "WeakAurasOptions" or addon == "DC-QOS") then
...
function OptionsPrivate.DropIndicator()
  ...
  if not indicator then
    ...
    groupTexture:SetTexture(OptionsPrivate.GetTexture("Square_FullWhite"))
