...
function HLBG.UpdateZoneState()
    ...
    if inHinterlands ~= HLBG._inHinterlands then
        ...
        if inHinterlands then
            ...
            C_Timer.After(0.5, function() HLBG.SafeRequest("STATUS") end)
            C_Timer.After(1.5, function() HLBG.SafeRequest("STATS") end)
            C_Timer.After(2.5, function() HLBG.SafeRequest("HISTORY", 1, 5, "id", "DESC") end)
