...
local function update()
  ...
  for i=1,count do
    ...
    -- Fallback: 3.3.5a often lacks IDs; scan the text
    if type(txt) == "string" then
      local name = txt:match("[Aa]ffix[:%s]+([%a%s]+)")
      if name and name ~= "" then
        name = (name:gsub("^%s+","")):gsub("%s+$","")
        label = "Affix: "..name
        -- try map icon by name
        for k,v in pairs(affixNames) do
          if v == name then icon = affixIcons[k]; break end
        end
        break
      end
    end
  ...
  elseif label == "" and HinterlandAffixHUDDB and HinterlandAffixHUDDB.lastAffix then
    ...
    f.text:SetText(line)
  elseif label == "" then
    -- visible fallback so users know the addon is active even if no data yet
    f.text:SetText("Affix: (waiting…)")
  end
  if icon then
...
local function onEvent(self, event, ...)
  if event == "ADDON_LOADED" then
    ...
    if name == "HinterlandAffixHUD" then
      if RegisterAddonMessagePrefix then pcall(RegisterAddonMessagePrefix, "HLBG") end
      print("HinterlandAffixHUD v"..HLAFFIXHUD_VERSION.." loaded")
      HinterlandAffixHUDDB = HinterlandAffixHUDDB or {}
  ...
  if event == "CHAT_MSG_ADDON" then
    ...
    -- Optional deterministic path if server sends addon messages
    if type(prefix) == "string" and prefix:find("HLBG", 1, true) and type(msg) == "string" then
      -- expected format examples:
      -- "AFFIX|Stormy|WEATHER|Rain 60%" or "AFFIX|Stormy"
      local aff = msg:match("AFFIX|([^|]+)")
      local w = msg:match("WEATHER|(.+)$")
      if aff and aff ~= "" then
        setAffixByName(aff)
      end
      if w and w ~= "" then
        HinterlandAffixHUDDB = HinterlandAffixHUDDB or {}
        HinterlandAffixHUDDB.lastWeather = w
        update()
      end
      return
    end
...
SlashCmdList["HLAFFIX"] = function(msg)
  ...
  elseif msg:find("^scale ") then
    local s = tonumber(msg:match("^scale%s+([%d%.]+)$") or "")
    if s and s >= 0.5 and s <= 2.0 then
      HinterlandAffixHUDDB = HinterlandAffixHUDDB or {}
      HinterlandAffixHUDDB.scale = s
      f:SetScale(s)
      print("HinterlandAffixHUD: scale set to", s)
    else
      print("Usage: /hlaffix scale <0.5..2.0>")
    end
    return
  elseif msg:find("^lock ") then
    local mode = msg:match("^lock%s+(%w+)$")
    HinterlandAffixHUDDB = HinterlandAffixHUDDB or {}
    if mode == "on" or mode == "1" then
      HinterlandAffixHUDDB.locked = true
      print("HinterlandAffixHUD: frame locked")
    elseif mode == "off" or mode == "0" then
      HinterlandAffixHUDDB.locked = false
      print("HinterlandAffixHUD: frame unlocked")
    else
      print("Usage: /hlaffix lock on|off")
    end
    return
  ...
  print("/hlaffix scale <0.5..2.0> - set HUD scale")
