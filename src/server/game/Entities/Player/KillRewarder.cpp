...
void KillRewarder::_RewardXP(Player* player, float rate)
{
    ...
    // Downgraded to DEBUG to avoid polluting normal server logs; can be raised when diagnosing.
    LOG_DEBUG("scripts", "InstrumentXP: KillRewarder -> player {} about to be given {} XP for victim {} (entry {}) hasAura800001={} pos=({:.1f},{:.1f},{:.1f}) map={}",
        player->GetName(), xp, _victim ? _victim->GetGUID().ToString() : ObjectGuid::Empty.ToString(), _victim ? _victim->GetEntry() : 0,
