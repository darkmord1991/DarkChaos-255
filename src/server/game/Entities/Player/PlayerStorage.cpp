...
InventoryResult Player::CanEquipItem(uint8 slot, uint16& dest, Item* pItem, bool swap, bool not_loading) const
{
    ...
    if (pItem)
    {
        ...
        if (pProto)
        {
            ...
            // MODIFIED: Skip this check for heirloom items to allow scaling to level 255
            if (ssd && ssd->MaxLevel < DEFAULT_MAX_LEVEL && ssd->MaxLevel < GetLevel() && pProto->Quality != ITEM_QUALITY_HEIRLOOM)
                return EQUIP_ERR_ITEM_CANT_BE_EQUIPPED;
