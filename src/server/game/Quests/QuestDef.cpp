...
{
    uint32 rewardLevel = level;

    if (rewardLevel > GT_MAX_LEVEL)
        rewardLevel = GT_MAX_LEVEL;
    ...
    if (GetRewHonorAddition() > 0 || GetRewHonorMultiplier() > 0.0f)
    {
        ...
        TeamContributionPointsEntry const* tc = sTeamContributionPointsStore.LookupEntry(rewardLevel);
        if (!tc)
            return 0;
