...
void PlayerbotsDatabaseConnection::DoPrepareStatements()
{
    ...
    PrepareStatement(PLAYERBOTS_SEL_RANDOM_BOTS_BY_OWNER_BOT_AND_EVENT, "SELECT `value`, `time`, validIn, `data` FROM playerbots_random_bots WHERE owner = ? AND bot = ? AND event = ? LIMIT 1", CONNECTION_SYNCH);
