...
void CharacterDatabaseConnection::DoPrepareStatements()
{
    ...
    PrepareStatement(CHAR_SEL_CHAR_REP_BY_FACTION, "SELECT standing FROM character_reputation WHERE faction = ? AND guid = ?", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_INS_CHAR_ACTION, "INSERT INTO character_action (guid, spec, button, action, type) VALUES (?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE action = VALUES(action), type = VALUES(type)", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_UPD_HLBG_PLAYER_DEATHS, "UPDATE dc_hlbg_player_stats SET total_deaths = total_deaths + 1 WHERE player_guid = ?", CONNECTION_ASYNC);
