...
// ---------------------------------------------------------------------------------
// DARKCHAOS CLONE -- map 821. Generated from boss_jarien_and_sothos.cpp, then renamed.
//
...
struct boss_jarien_dc : public BossAI
{
    boss_jarien_dc(Creature* creature) : BossAI(creature, DATA_JARIEN)
    {
...
struct boss_sothos_dc : public BossAI
{
    boss_sothos_dc(Creature* creature) : BossAI(creature, DATA_SOTHOS)
    {
        _scheduler.SetValidator([this]
            {
                return !me->HasUnitState(UNIT_STATE_CASTING);
...
{
    RegisterStratholmeDCCreatureAI(boss_jarien_dc);
    RegisterStratholmeDCCreatureAI(boss_sothos_dc);
}
