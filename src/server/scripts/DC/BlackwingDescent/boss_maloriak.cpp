        ...
        while (uint32 eventId = events.ExecuteEvent())
        {
            switch (eventId)
            {
                case EVENT_ARCANE_STORM:
                {
                    ...
                    me->m_Events.AddEventAtOffset([maloriak]() { ; }, 6s + 500ms);
    ...
    void DoAction(int32 action) override
    {
        switch (action)
        {
            case ACTION_RELEASE_EXPERIMENT:
            {
                ...
                {
                    chamber->SetGameObjectFlag(GO_FLAG_IN_USE);
    ...
    void DoAction(int32 action) override
    {
        switch (action)
        {
            ...
            case ACTION_MALORIAK_DEAD:
                // [AC 3.3.5 downport] AnimKit unsupported: me->SetAIAnimKitId(AI_ANIM_KIT_ID_LORD_VICTOR_NEFARIUS);
                _events.Reset();
...
class spell_maloriak_flash_freeze_dummy : public SpellScript
{
    ...
    void FilterTargets(std::list<WorldObject*>& targets)
    {
        ...
        targets.remove_if([caster](WorldObject* obj)
        {
            ...
            return target->isDead() || !target->ToTempSummon() || target->ToTempSummon()->GetSummonerUnit() != caster;
        });
...
class spell_maloriak_release_experiments : public SpellScript
{
    ...
    void FilterTargets(std::list<WorldObject*>& targets)
    {
        ...
        targets.remove_if(Acore::UnitAuraCheck(false, SPELL_DROWNED_STATE));
