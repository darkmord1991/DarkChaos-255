        ...
        while (uint32 eventId = _events.ExecuteEvent())
        {
            switch (eventId)
            {
                ...
                case EVENT_CHAIN_LIGHTNING:
                    if (Unit* target = SelectTarget(SelectTargetMethod::Random, 0, 90.f, true))
...
class spell_bwd_execution_sentence : public AuraScript
{
    ...
    {
        if (GetTargetApplication()->GetRemoveMode() == AURA_REMOVE_BY_CANCEL || GetTargetApplication()->GetRemoveMode() == AURA_REMOVE_BY_DEATH)
...
class spell_bwd_stoneblood : public SpellScript
{
    ...
    void HandleScriptEffect(SpellEffIndex /*effIndex*/)
    {
        ...
        {
            (*itr)->Remove(AURA_REMOVE_BY_ENEMY_SPELL);
...
class at_bwd_intro: public OnlyOnceAreaTriggerScript
{
    public:
        ...
        bool _OnTrigger(Player* player, AreaTrigger const* /*areaTrigger*/)
        {
