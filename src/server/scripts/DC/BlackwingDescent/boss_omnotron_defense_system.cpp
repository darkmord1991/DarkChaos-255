        ...
        while (uint32 eventId = events.ExecuteEvent())
        {
            switch (eventId)
            {
                ...
                case EVENT_POWER_UP_FIRST_GOLEM:
                    ...
                    Acore::Containers::RandomShuffle(_golemGuidVector);
                    SelectNextGolemGUID();
...
struct npc_omnotron_electron : public ScriptedAI
{
    ...
    void MovementInform(uint32 type, uint32 pointId) override
    {
        ...
        {
            me->GetMotionMaster()->MovePath(BOSS_OMNOTRON * 100);
            me->SetWalk(true);
        }
    }
...
struct npc_omnotron_poison_bomb : public ScriptedAI
{
    ...
    void IsSummonedBy(WorldObject* summoner) override
    {
        ...
        SelectTargetList(targets, 25, SelectTargetMethod::Random, 0, 100.0f, true);

        if (targets.empty())
            return;

        std::list<Unit*> targetsCopy = targets;
        targets.remove_if(DistanceCheck(summoner->ToUnit()));
