...
uint32 const MapBlackwingDescent = 669;
...
// Cata compatibility stub: the 3.3.5 CreatureAI has no OnSpellCastFinished hook, so the ported
// boss AIs keep those method bodies as (never-invoked) dead code. This enum lets them compile.
enum SpellFinishReason
{
    SPELL_FINISHED_SUCCESSFUL_CAST      = 0,
    SPELL_FINISHED_UNSUCCESSFUL_CAST    = 1,
    SPELL_FINISHED_CHANNELING_COMPLETE  = 2,
    SPELL_FINISHED_CANCELED             = 3
};

enum BWDSharedSpells
{
    // Room Stalker
...
enum BWDWorldStates
{
    ...
    // Magmaw
    WORLD_STATE_ID_PARASITE_EVENING         = 5640,

    // Chimaeron
    WORLD_STATE_ID_FULL_OF_SOUND_AND_FURY   = 5643,

    // Atramedes
    WORLD_STATE_ID_SILENCE_IS_GOLDEN        = 5655,

    // Nefarian's End
    WORLD_STATE_ID_KEEPING_IT_IN_THE_FAMILY = 5652
};
