...
#include "PassiveAI.h"
#include "TemporarySummon.h"  // was PhasingHandler.h (Cata+, no 3.3.5 equiv)
...
// [3.3.5] The elevator-group adds are authored at the lowered (lava) elevator-center z; the raised
// elevator surface is NefarianElevatorCenterPosition.z (6.488, where Nefarian lands). This fork has no
// scripted transport-passenger API, so SetupTransportSpawns lifts the adds by (raised - lowered) so they
// sit on the raised elevator instead of the lava. True ride-up staging needs the transport subsystem.
float const ELEVATOR_LOWERED_CENTER_Z = -7.33029f;
...
struct boss_nefarians_end : public BossAI
{
    ...
    void JustRespawned() override
    {
        ...
        else
        {
            ...
            ; // [3.3.5] MoveCyclicPath unsupported
    ...
    void EnterEvadeMode(EvadeReason /*why*/) override
    {
        ...
        if (events.IsInPhase(PHASE_TWO) && !_elevatorLowered)
            instance->SetData(DATA_RESET_ELEVATOR, events.GetTimeUntilEvent(EVENT_ELEVATOR_LOWERED).count());
        else if (GameObject* transport = GetElevator())
            transport->SetGoState(GOState(GO_STATE_ACTIVE));
        ...
        while (uint32 eventId = events.ExecuteEvent())
        {
            switch (eventId)
            {
                ...
                case EVENT_ANNOUNCE_AIR_CRACKLES:
                    Talk(SAY_ANNOUNCE_AIR_CRACKLES);
                    me->GetMotionMaster()->MovePoint(POINT_LIFTOFF, NefarianLiftOffPosition);
                    break;
                case EVENT_PREPARE_LANDING:
                {
                    ...
                    me->GetMotionMaster()->MovePoint(POINT_PREPARE_LANDING, pos);
                    break;
                }
                ...
                case EVENT_ELECTROCUTE:
                    ...
                    if (Creature* onyxia = instance->GetCreature(DATA_ONYXIA))
                        if (Aura* charge = onyxia->GetAura(SPELL_ELECTRICAL_CHARGE_ONYXIA))
                            charge->ModStackAmount(17, AURA_REMOVE_BY_DEFAULT);
                    break;
                case EVENT_DOMINION:
...
private:
    // [3.3.5] TC bound these summons to the elevator as transport passengers (so they render
    // moving with it). This fork has no scripted transport-passenger API for the type-11
    // elevator, so we summon them at their authored *world* positions (cata_world stores world
    // coords, at the lowered/lava elevator center). For the elevator group we lift them by the
    // elevator's travel distance so they land on the raised elevator surface — reachable by the
    // players and by Nefarian (who lands at z 6.488). The visual "ride up" is the accepted loss;
    // see PORT_NOTES.md. The summons still join the boss's summon list via JustSummoned.
    void SetupTransportSpawns(uint32 summonGroupId)
    {
        if (!instance->GetGameObject(DATA_BLACKWING_ELEVATOR_ONYXIA))
            return;
        ...
        float const raiseZ = (summonGroupId == SUMMON_GROUP_ELEVATOR)
            ? (NefarianElevatorCenterPosition.GetPositionZ() - ELEVATOR_LOWERED_CENTER_Z)
            : 0.0f;

        for (TempSummonData const& data : *summonGroupData)
        {
            Position pos = data.pos;
            pos.m_positionZ += raiseZ;
            me->SummonCreature(data.entry, pos, data.type, data.time);
        }
    }
    ...
    void EnterPhaseThree()
    {
        ...
        if (GameObject* elevator = GetElevator())
            elevator->SetGoState(GOState(GO_STATE_ACTIVE));
        ...
        switch (action)
        {
            ...
            case ACTION_UPDATE_ELECTRICAL_CHARGE:
                if (Aura* chargeAura = me->GetAura(SPELL_ELECTRICAL_CHARGE_ONYXIA))
                {
                    ...
                    me->SetPower(POWER_ENERGY, stacks - 1);
                }
                break;
        ...
        while (uint32 eventId = _events.ExecuteEvent())
        {
            switch (eventId)
            {
                case EVENT_JUMP_DOWN_TO_PLATFORM:
                    // [3.3.5] The jump destination is resolved into world space by
                    // spell_nefarians_end_jump_down_to_platform (elevator-frame transform), so
                    // the cast fires unconditionally instead of being gated on transport-passenger
                    // binding (which trapped it in dead code and stopped the prototypes engaging).
        ...
        switch (action)
        {
            case ACTION_SPREAD_FLAMES:
            {
                ...
                if (!_summonedByController && me->GetEntry() != NPC_SHADOWBLAZE_FLASHPOINT)
                {
                    ...
                    me->CastSpell(summonPos.GetPositionX(), summonPos.GetPositionY(), z, SPELL_BRUSHFIRE_SUMMON, true);
                }
                else
                {
                    if (me->GetEntry() == NPC_SHADOWBLAZE_FLASHPOINT)
                    {
                        me->CastSpell(me->GetPositionX() - 5.f, me->GetPositionY(), z, SPELL_BRUSHFIRE_SUMMON, true);
                        me->CastSpell(me->GetPositionX(), me->GetPositionY() + 5.f, z, SPELL_BRUSHFIRE_SUMMON, true);
                    }
                    else if (_summonedByController)
                    {
                        for (uint8 i = 0; i < 2; i++)
                        {
                            ...
                            me->CastSpell(x, y, z, SPELL_BRUSHFIRE_SUMMON, true);
                        }
                    }
                }
                break;
        ...
        switch (target->GetEntry())
        {
            case BOSS_NEFARIAN:
                if (target->GetReactState() == REACT_AGGRESSIVE)
                {
                    ...
                    {
                        if (Aura* charge = onyxia->GetAura(SPELL_ELECTRICAL_CHARGE_ONYXIA))
                            charge->ModStackAmount(1, AURA_REMOVE_BY_EXPIRE);
                    }
                }
                break;
            case NPC_ONYXIA:
...
class spell_nefarians_end_lightning_discharge_damage : public SpellScript
{
    ...
    void FilterTargets(std::list<WorldObject*>& targets)
    {
        ...
        targets.remove_if([spell, caster](WorldObject const* obj)->bool
        {
            ...
            return !target || const_cast<Unit*>(target)->IsImmunedToSpell(spell, caster);
        });
    }
...
class spell_nefarians_end_animate_bones_dummy : public SpellScript
{
    ...
    void HandleHit(SpellEffIndex /*effIndex*/)
    {
        ...
        target->AttackStop();
        target->GetThreatMgr().ClearAllThreat();
...
class spell_nefarians_end_jump_down_to_platform : public SpellScript
{
    ...
    void SetDest(SpellDestination& dest)
    {
        ...
        // [3.3.5] ChromaticPrototypeJumpPositions are transport-relative offsets. With no scripted
        // passenger API (and TransportBase::CalculatePassengerPosition being protected), inline the
        // same transform: 2D-rotate the offset by the elevator orientation, translate by its position.
        float const transO = elevator->GetOrientation();
        float const cosO = std::cos(transO);
        float const sinO = std::sin(transO);
        for (uint8 i = 0; i < MaxChromaticPrototypes; i++)
        {
            float const inx = ChromaticPrototypeJumpPositions[i].GetPositionX();
            float const iny = ChromaticPrototypeJumpPositions[i].GetPositionY();
            float const wx = elevator->GetPositionX() + inx * cosO - iny * sinO;
            float const wy = elevator->GetPositionY() + iny * cosO + inx * sinO;
            float const wz = elevator->GetPositionZ() + ChromaticPrototypeJumpPositions[i].GetPositionZ();
            positions[i].Relocate(wx, wy, wz,
                Position::NormalizeOrientation(transO + ChromaticPrototypeJumpPositions[i].GetOrientation()));
        }
...
{
        PrepareSpellScript(spell_nefarians_end_onyxia_start_fight_2_effect);
    bool Validate(SpellInfo const* /*spellInfo*/) override
    {
        return ValidateSpellInfo({ SPELL_SHADOW_OF_COWARDICE_DAMAGE });
    }
...
{
        PrepareAuraScript(spell_nefarians_end_empowering_strikes);
    void HandlePeriodicTick(AuraEffect const* /*aurEff*/)
    {
        PreventDefaultAction();
    ...
    void Register() override
    {
        OnEffectPeriodic += AuraEffectPeriodicFn(spell_nefarians_end_empowering_strikes::HandlePeriodicTick, EFFECT_0, SPELL_AURA_PERIODIC_TRIGGER_SPELL);
    }
};

class spell_nefarians_end_brushfire_pre_start_periodic : public AuraScript
{
        PrepareAuraScript(spell_nefarians_end_brushfire_pre_start_periodic);
    bool Load() override
    {
...
{
        PrepareSpellScript(spell_nefarians_end_brushfire_start);
    ...
    void Register() override
    {
        OnEffectHitTarget += SpellEffectFn(spell_nefarians_end_brushfire_start::HandleDummyEffect, EFFECT_0, SPELL_EFFECT_DUMMY);
    }
};
...
class spell_nefarians_end_dominion : public AuraScript
{
    ...
    void AfterApply(AuraEffect const* /*aurEff*/, AuraEffectHandleModes /*mode*/)
    {
        ...
        target->SetByteFlag(UNIT_FIELD_BYTES_2, 1, UNIT_BYTE2_FLAG_UNK1);
    }
    ...
    void AfterRemove(AuraEffect const* /*aurEff*/, AuraEffectHandleModes /*mode*/)
    {
        ...
        target->RemoveByteFlag(UNIT_FIELD_BYTES_2, 1, UNIT_BYTE2_FLAG_UNK1);
    }
