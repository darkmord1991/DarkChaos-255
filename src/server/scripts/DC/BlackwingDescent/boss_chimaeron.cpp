...
enum GossipMenuMisc
{
    // DC-authored menu id (06_text.sql wires creature_template.gossip_menu_id = 11812 / gossip_menu MenuID 11812).
    // CataTC hardcoded retail's 11837 here; the core passes the DB menu id to sGossipSelect, so match the SQL.
    GOSSIP_MENU_ID_RESTRICTIONS = 11812,
        ...
        while (uint32 eventId = events.ExecuteEvent())
        {
            switch (eventId)
            {
                case EVENT_CAUSTIC_SLIME:
                    if (SelectTarget(SelectTargetMethod::Random, 0, NonTankTargetSelector(me)))
...
struct npc_chimaeron_finkle_einhorn : public ScriptedAI
{
    ...
    // 3.3.5 creature gossip hook (void sGossipSelect); the core passes (menuId, gossipListId) as (sender, action).
...
class spell_chimaeron_caustic_slime_targeting : public SpellScript
{
    ...
    void FilterTargets(std::list<WorldObject*>& targets)
    {
        ...
        if (targets.size() > size)
            Acore::Containers::RandomResize(targets, size);
    }
...
class spell_chimaeron_caustic_slime: public SpellScript
{
    ...
    void SplitDamage(SpellMissInfo)
    {
        if (!_targetCount)
            return;
    ...
    void Register() override
    {
        ...
        BeforeHit += BeforeSpellHitFn(spell_chimaeron_caustic_slime::SplitDamage);
    }
