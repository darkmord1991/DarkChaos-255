    ...
    void JustSummoned(Creature* summon) override
    {
        ...
        switch (summon->GetEntry())
        {
            case NPC_SONAR_PULSE:
                ...
                {
                    Unit* summoner = summon->ToTempSummon()->GetSummonerUnit();
                    if (!summoner)
                        return;
                    ...
                    summon->m_Events.AddEventAtOffset([summon, summoner]()
                    {
                        ...
                        summon->GetMotionMaster()->MovePoint(POINT_NONE, pos);
                        if (uint32 duration = summon->movespline->Duration())
                            summon->DespawnOrUnsummon(Milliseconds(duration));
                    }, 800ms);
                }, 400ms);
                break;
    ...
    void DoAction(int32 action) override
    {
        switch (action)
        {
            case ACTION_START_INTRO:
                me->GetMotionMaster()->MovePoint(POINT_CAST_ROARING_BREATH, IntroFlightPosition1);
                break;
            default:
                break;
        }
        ...
        while (uint32 eventId = events.ExecuteEvent())
        {
            switch (eventId)
            {
                ...
                case EVENT_FLY_TO_INTRO_LAND_POSITION:
                    me->GetMotionMaster()->MovePoint(POINT_PREPARE_LAND_INTRO, IntroFlightPosition2);
                    break;
    ...
    void DoAction(int32 action) override
    {
        switch (action)
        {
            case ACTION_PLAYER_ENTERED:
                ...
                // [AC 3.3.5 downport] AnimKit unsupported: me->SetAIAnimKitId(AI_ANIM_KIT_ID_OBNOXIOUS_IMP);
                break;
            case ACTION_PLAYER_LEFT:
                DoCastSelf(SPELL_PHASE_SHIFT, true);
                _events.Reset();
                _events.ScheduleEvent(EVENT_FOCUS_PLAYER, 1s);
                // [AC 3.3.5 downport] AnimKit unsupported: me->SetAIAnimKitId(0);
                break;
            default:
                break;
        }
        ...
        while (uint32 eventId = _events.ExecuteEvent())
        {
            switch (eventId)
            {
                ...
                case EVENT_FOCUS_PLAYER:
                    if (Unit* target = SelectTarget(SelectTargetMethod::Random, 0, 100.0f, true, true, -SPELL_PESTERED))
...
private:
    ...
    void trackTarget(Unit* target)
    {
        ...
        me->GetMotionMaster()->MoveFollow(target, 0.0f, 0.f);
...
class spell_atramedes_modulation : public SpellScript
{
    ...
    void ChangeDamage(SpellEffIndex /*effIndex*/)
    {
        ...
        AddPct(damage, target->GetPower(POWER_ENERGY));
        SetHitDamage(damage);
    }
...
class spell_atramedes_sound_bar : public AuraScript
{
    ...
    void HandleNoisyAura(AuraEffect const* aurEff)
    {
        ...
        else if (target->GetPower(POWER_ENERGY) >= 50)
            if (!0)
            ...
            if (atramedes->IsAIEnabled)
            {
                if (InstanceScript* instance = atramedes->GetInstanceScript())
                {
                    if (instance->GetData(DATA_IS_IN_INTRO_PHASE))
                        atramedes->GetMotionMaster()->MovePoint(POINT_PREPARE_LAND_INTRO, IntroFlightPosition2);
...
class spell_atramedes_sonic_breath : public SpellScript
{
    ...
    void FilterTargets(std::list<WorldObject*>& targets)
    {
        ...
        targets.remove_if([this](WorldObject* target){ Unit* c = GetCaster(); return c && target == c->GetVictim(); });
