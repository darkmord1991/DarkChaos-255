...
// DarkChaos downport of the Cataclysm raid Blackwing Descent (map 669).
// Ported from the TrinityCore-family "CataTC" implementation; adapted to
// AzerothCore 3.3.5a idioms. Notable AC deltas vs the source:
//   * TC InstanceScript::Create()  -> AC InstanceScript::Initialize()
//   * TC Unit::IsAIEnabled()       -> AC bool member Unit::IsAIEnabled
//   * TC RemoveFlag(GAMEOBJECT_FLAGS, ...) -> AC typed RemoveGameObjectFlag(...)
//   * TC GO_STATE_TRANSPORT_ACTIVE  has no 3.3.5 equivalent (see EVENT_RAISE_ELEVATOR TODO)
...
{
public:
    instance_blackwing_descent() : InstanceMapScript(BWDScriptName, MapBlackwingDescent) { }
    ...
    struct instance_blackwing_descent_InstanceMapScript : public InstanceScript
    {
        ...
        // AC calls Initialize() on instance creation (TC used Create()).
        // NOTE: Initialize() runs on load too (before Load()); ReadSaveDataMore()
        // below despawns these baseline groups first to avoid double-spawn.
        ...
        void SetData(uint32 type, uint32 data) override
        {
            switch (type)
            {
                ...
                case DATA_RESET_ELEVATOR:
                    _events.ScheduleEvent(EVENT_RAISE_ELEVATOR, Milliseconds(data));
                    break;
                default:
                    break;
            }
                ...
                switch (eventId)
                {
                    case EVENT_MAKE_ANCIENT_BELL_SELECTABLE:
                        if (GameObject* bell = GetGameObject(DATA_ANCIENT_BELL))
                            bell->RemoveGameObjectFlag(GO_FLAG_NOT_SELECTABLE);
                    ...
                    case EVENT_RAISE_ELEVATOR:
                        ...
                        {
                            // TODO(BWD): CataTC raised the Onyxia platform via
                            // GO_STATE_TRANSPORT_ACTIVE, which this 3.3.5 fork lacks.
                            // Approximated with GO_STATE_ACTIVE; validate the elevator
                            // animation + Onyxia passenger sync in-game (transport risk).
        ...
        void ReadSaveDataMore(std::istringstream& data) override
        {
            ...
            // Initialize() already spawned the baseline groups; clear them so the
            // saved-state logic re-establishes the correct set without duplicates.
            instance->SpawnGroupDespawn(SPAWN_GROUP_ANCIENT_DWARVEN_SHIELDS);
            instance->SpawnGroupDespawn(SPAWN_GROUP_DWARVEN_SPIRITS_LEFT);
            instance->SpawnGroupDespawn(SPAWN_GROUP_DWARVEN_SPIRITS_RIGHT);
