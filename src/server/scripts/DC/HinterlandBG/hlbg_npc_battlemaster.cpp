...
public:
    ...
    bool OnGossipHello(Player* player, Creature* creature) override
    {
        ...
        if (hl->IsPlayerQueued(player))
            AddGossipItemFor(player, GOSSIP_ICON_BATTLE, "Leave Hinterlands BG Queue", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 2);
        else
            AddGossipItemFor(player, GOSSIP_ICON_BATTLE, "Join Hinterlands BG Queue", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 1);

        AddGossipItemFor(player, GOSSIP_ICON_CHAT, "What is Hinterlands BG?", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 3);
