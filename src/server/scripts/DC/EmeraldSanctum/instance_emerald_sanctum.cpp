...
//
// Two encounters only: Erennius, then "The Wakener" -- one of four corrupted green dragons,
// decided by the week. All four share DungeonEncounter 1121 so the M+ HUD and boss counters
// read 2 whichever is present.
//
...
// An instanced raid needs the decision made per instance, which is what this does.
//
...
// and rolls its own pools. Child pools never auto-spawn, so the four Wakener pools stay
// dormant until this picks one.
...
// Path-qualified: the per-script include dirs do not put Seasons/ on this folder's search
// path. GreatVault, MythicPlus, CrossSystem and SeasonalRewardSystem all include it this way.
...
namespace
{
    ...
    /// the lockouts, so "this week's Wakener" flips when everything else resets.
...
{
public:
    instance_emerald_sanctum() : InstanceMapScript(EmeraldSanctumScriptName, MapEmeraldSanctum) { }
    ...
    struct instance_emerald_sanctum_InstanceMapScript : public InstanceScript
    {
        ...
        {
            SetHeaders(EMSADataHeader);
            SetBossNumber(EmeraldSanctumEncounterCount);
        ...
        {
            if (_wakenerSpawned || !instance)
                return;

            // Clear whatever the parent pool may have rolled, then force this week's child.
            sPoolMgr->DespawnPool(instance->GetPoolData(), POOL_WAKENER_PARENT);
            sPoolMgr->SpawnPool(instance->GetPoolData(), POOL_WAKENER_FIRST + _wakenerIndex);
            _wakenerSpawned = true;

            LOG_DEBUG("scripts.dc", "Emerald Sanctum instance {}: week index {} -> pool {}",
                instance->GetInstanceId(), _wakenerIndex, POOL_WAKENER_FIRST + _wakenerIndex);
        }
        ...
        {
            // Persisted so a saved instance keeps the Wakener it was created with, even if the
            // week rolls over while a raid is still saved to it.
            data << _wakenerIndex << ' ';
        }

    private:
        uint32 _wakenerIndex = 0;
        bool _wakenerSpawned = false;
    };
