...
#ifndef DEF_DC_NAXX40_INSTANCE_H
#define DEF_DC_NAXX40_INSTANCE_H

#include "naxx40_defs.h"

#define DataHeader "NAX"

#define NaxxramasScriptName "instance_naxxramas_40"
...
static constexpr uint32 NaxxramasMapId            = 2921;  // DC: SoD Naxxramas map; 533 stays stock WotLK
...
// DC: the 40-man runs `instance_naxxramas_40` on map 2921, but some of the AIs
// below are attached to creature entries that stock Naxxramas also spawns on map
// 533, which runs core's `instance_naxxramas`. GetInstanceAI compares the script
// NAME, so accept either -- that is what makes the vanilla mechanics apply on
// both maps rather than silently falling back to a default AI on 533.
template<typename AI, typename T>
inline AI* GetNaxxramasAI(T* obj)
{
    if (AI* ai = GetInstanceAI<AI>(obj, NaxxramasScriptName))
        return ai;

    return GetInstanceAI<AI>(obj, "instance_naxxramas");
}
