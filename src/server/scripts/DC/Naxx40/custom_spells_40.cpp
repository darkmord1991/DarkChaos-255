...
class spell_gen_consumption : public SpellScript
{
    ...
    void CalculateDamage(SpellEffIndex /*effIndex*/)
    {
        ...
        else if (map->GetId() == MAP_NAXX || map->GetId() == 533) // NAXX10 N / NAXX40
        // DC: 533 is stock WotLK Naxxramas, 2921 (MAP_NAXX) is the 40-man.
        // Both are covered so the vanilla tuning applies on either map.
        {
...
class isAllowedToCastSpell : public SpellScript
{
    ...
    SpellCastResult CheckCorruptedMind()
    {
        ...
        // Only enforce the check in Naxxramas 10HC (DC: map 2921, 10-man heroic).
        if (map->GetId() != MAP_NAXX || map->GetDifficulty() != RAID_DIFFICULTY_10MAN_HEROIC)
            return SPELL_CAST_OK;
