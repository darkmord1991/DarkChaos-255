...
namespace ShadowfangKeepCata::LordGodfrey
{
enum Texts
{
...
struct boss_lord_godfrey : public BossAI
{
    ...
    // NOTE(port): CataTC's OnSpellCastFinished did nothing here but clear the
    // TC-only spell-focus lock (MakeInterruptable), which AC has no equivalent
    // for -- the override collapsed to a no-op and was dropped.
...
void AddSC_boss_lord_godfrey()
{
    ...
    using namespace ShadowfangKeepCata::LordGodfrey;
    RegisterShadowfangKeepCataCreatureAI(boss_lord_godfrey);
