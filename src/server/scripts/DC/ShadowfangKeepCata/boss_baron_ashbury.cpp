...
namespace ShadowfangKeepCata::BaronAshbury
{
enum Spells
{
    // Baron Ashbury
...
struct boss_baron_ashbury : public BossAI
{
    ...
    void HealReceived(Unit* healer, uint32& /*heal*/) override
    {
        ...
        // NOTE(port): AC has no InstanceMap::GetWorldStateValue, so the "already
        // set" guard is kept on the AI instead of read back off the map.
        if (!_pardonDenied)
        {
            _pardonDenied = true;
    ...
    // NOTE(port): CataTC used a single OnSpellCastFinished(spell, reason) and
    // filtered out the "channel started" case by hand. AC has no such hook but
    // splits the same signal in two: OnChannelFinished fires when a channel
    // really ends (Pain and Suffering, Heroic Stay of Execution) and OnSpellCast
    // when an ordinary cast completes (Normal Stay of Execution). Routing both
    // into one handler reproduces the original behaviour exactly.
    void HandleCastFinished(SpellInfo const* spell)
    {
        ...
        while (uint32 eventId = events.ExecuteEvent())
        {
            switch (eventId)
            {
                ...
                case EVENT_PAIN_AND_SUFFERING:
                    ...
                    if (IsHeroic() && events.GetTimeUntilEvent(EVENT_ASPHYXIATE) > 15s)
    ...
    void HandleTriggerSpell(AuraEffect const* aurEff)
    {
        ...
        if (Unit* caster = GetCaster())
        {
            if (Unit* target = GetTarget())
            {
                ...
                int32 bp0 = int32(damage);
                target->CastCustomSpell(target, triggerSpell, &bp0, nullptr, nullptr, true,
                    nullptr, aurEff, caster->GetGUID());
            }
        }
    }
...
void AddSC_boss_baron_ashbury()
{
    ...
    using namespace ShadowfangKeepCata::BaronAshbury;
    RegisterShadowfangKeepCataCreatureAI(boss_baron_ashbury);
