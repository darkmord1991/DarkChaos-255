...
namespace ShadowfangKeepCata::LordWalden
{
enum Texts
{
    SAY_AGGRO   = 0,
...
class spell_walden_ice_shards : public AuraScript
{
    ...
    void HandleEffectPeriodic(AuraEffect const* aurEff)
    {
        ...
        if (caster->IsAIEnabled)
            if (Unit* target = caster->AI()->SelectTarget(SelectTargetMethod::Random, 0, 50.0f, true))
...
void AddSC_boss_lord_walden()
{
    ...
    using namespace ShadowfangKeepCata::LordWalden;
    RegisterShadowfangKeepCataCreatureAI(boss_lord_walden);
