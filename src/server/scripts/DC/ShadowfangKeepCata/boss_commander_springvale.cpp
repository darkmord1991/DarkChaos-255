...
namespace ShadowfangKeepCata::CommanderSpringvale
{
enum Spells
{
...
public:
            ...
            while(uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
                {
                    ...
                    case EVENT_WORD_OF_SHAME:
                        me->RemoveAurasDueToSpell(SPELL_UNHOLY_POWER);
                        me->RemoveAurasDueToSpell(SPELL_UNHOLY_POWER_HC);
                        if (Unit* target = SelectTarget(SelectTargetMethod::Random, 0, 100.0f, true, true, -SPELL_WORD_OF_SHAME))
    ...
    {
        return GetShadowfangKeepCataAI<boss_commander_springvaleAI>(creature);
    }
};
...
public:
    ...
    CreatureAI* GetAI(Creature* creature) const override
    {
        return GetShadowfangKeepCataAI<npc_wailing_guardsmanAI>(creature);
    }
};
...
public:
    ...
    CreatureAI* GetAI(Creature* creature) const override
    {
        return GetShadowfangKeepCataAI<npc_tormented_officerAI>(creature);
    }
};
    ...
    class spell_sfk_unholy_power_SpellScript : public SpellScript
    {
        ...
        void HandleStacks()
        {
            if (Unit* target = GetHitUnit())
            {
                ...
                if (!aura)
                    aura = target->GetAura(SPELL_UNHOLY_POWER);
        ...
        void HandleStacks()
        {
            if (Creature* target = GetHitCreature())
            {
                ...
                if (aura && aura->GetStackAmount() == 3 && target->IsAIEnabled)
                    target->AI()->DoAction(ACTION_UNHOLY_POWER);

                // NOTE(port): AC has no InstanceMap::GetWorldStateValue. This sits in a
                // spell script with no instance-side state to hang a flag on, and the
                // guard only suppressed a duplicate packet -- the written value is the
                // same either way, and the stack == 3 test above already makes this rare.
...
void AddSC_boss_commander_springvale()
{
    ...
    using namespace ShadowfangKeepCata::CommanderSpringvale;
