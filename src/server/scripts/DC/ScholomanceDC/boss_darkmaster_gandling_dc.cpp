...
// ---------------------------------------------------------------------------------
// DARKCHAOS CLONE -- map 822. Generated from boss_darkmaster_gandling.cpp, then renamed.
//
...
//
//     * Gandling's room teleport no longer casts the six shadow-portal spells. Those carry
//       spell_target_position rows hardcoded to MapID 289 and would throw a PLAYER into
//       stock Scholomance. See the note at the call site.
...
class boss_darkmaster_gandling_dc : public CreatureScript
{
public:
    boss_darkmaster_gandling_dc() : CreatureScript("boss_darkmaster_gandling_dc") {}

    CreatureAI* GetAI(Creature* creature) const override
    {
        return GetScholomanceDCAI<boss_darkmaster_gandling_dcAI>(creature);
    }

    struct boss_darkmaster_gandling_dcAI : public BossAI
    {
        boss_darkmaster_gandling_dcAI(Creature* creature) : BossAI(creature, DATA_DARKMASTER_GANDLING), summons(me) { }
        ...
        void SpellHitTarget(Unit* target, SpellInfo const* spellinfo) override
        {
            ...
            if (spellinfo && spellinfo->Id == SPELL_SHADOW_PORTAL && target)
            {
                ...
                // -------------------------------------------------------------------
                // THE CROSS-INSTANCE LEAK THIS CLONE HAS TO CLOSE
                // -------------------------------------------------------------------
                // Upstream casts one of six shadow-portal spells here. Each takes its
                // destination from `spell_target_position`, and every one of those rows
                // hardcodes MapID 289.
                //
                // Spell.cpp:1403-1409 applies that map id ABSOLUTELY for
                // SPELL_EFFECT_TELEPORT_UNITS, and the target here is a PLAYER -- so on map
                // 822 the cast is a real teleport out of this instance and into stock
                // Scholomance. (Stratholme had the same shape of bug, but its equivalent
                // spells were creature-targeted, so SpellEffects.cpp:1240 caught them and
                // only logged. This one would actually move a player.)
                //
                // spell_target_position is keyed by spell id alone and cannot hold a
                // different map per dungeon, so the fix is to teleport directly to the
                // coordinates the spell would have used. GandlingRoomDestinations lives in
                // scholomance_dc.h in the SAME ORDER as the upstream spell table.
                //
                // Cost: the spell's visual is lost. That is cheaper than cloning six spells
                // into the 234-field fork Spell.dbc and redeploying the client.
                if (Player* player = target->ToPlayer())
                {
                    GandlingRoomDest const& dest = GandlingRoomDestinations[room];
                    player->TeleportTo(MAP_SCHOLOMANCE_DC, dest.x, dest.y, dest.z, dest.o);
                }
...
class npc_risen_guardian_dc : public CreatureScript
{
public:
    npc_risen_guardian_dc() : CreatureScript("npc_risen_guardian_dc") {}

    struct npc_risen_guardian_dcAI : public ScriptedAI
    {
        npc_risen_guardian_dcAI(Creature* creature) : ScriptedAI(creature)
        {
    ...
    CreatureAI* GetAI(Creature* creature) const override
    {
        return GetScholomanceDCAI<npc_risen_guardian_dcAI>(creature);
    }
};

void AddSC_boss_darkmaster_gandling_dc()
{
    new boss_darkmaster_gandling_dc();
    new npc_risen_guardian_dc();
}
